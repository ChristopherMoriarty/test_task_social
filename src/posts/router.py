...
async def get_posts(
    ...
    try:
        post_data = await get_all_posts(session=session)
        ...
        await basic_exception(status_code=500, message="Error while getting posts")
...
@router.post("/me/{post_id}/like", response_model=dict)
async def make_like(
    post_id: int,
    session: AsyncSession = Depends(get_async_session),
    user: User = Depends(current_user),
...
@router.delete("/me/{post_id}/unlike", response_model=dict)
